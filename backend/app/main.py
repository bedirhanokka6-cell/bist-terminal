...
app = FastAPI(title='BIST Terminal API', version='0.2.1')
...
def data_freshness(index_value):
    """
    Son mumun zamanını ve yaklaşık yaşını hesaplar.
    Bu değer gerçek veri sağlayıcısının kesin gecikmesi değil,
    son alınan mumun ne kadar eski olduğunu gösterir.
    """
...
def save_signal(
    ...
    """
    Seçili hisseyi o an analiz eder
    ve sonucu PostgreSQL'e kaydeder.
    """
...
def evaluate_one_signal(
    ...
    return {
        ...
        'message':
            ...
            'Henüz yeterli işlem günü geçmedi.',
...
def signal_stats(
    ...
    q = (
        db.query(SignalResult)
        .filter(
            SignalResult.horizon_days ==
            horizon_days,

            SignalResult.evaluated_at
            .isnot(None),
        )
    )
    ...
    return {
        ...
        'note':
            'BEKLE sinyalleri başarı oranına dahil edilmez.',
    }